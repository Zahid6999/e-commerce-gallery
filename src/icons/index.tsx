import { cn } from '@/utils/cn';
import { FC } from 'react';

interface ArrowIconProps {
  className?: string;
}

const FacebookIcon: FC<ArrowIconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="none"
    className={cn(className)}>
    <path
      d="M24 12.0301C24 5.38947 18.624 0 12 0C5.376 0 0 5.38947 0 12.0301C0 17.8526 4.128 22.7008 9.6 23.8195V15.6391H7.2V12.0301H9.6V9.02256C9.6 6.70075 11.484 4.81203 13.8 4.81203H16.8V8.42105H14.4C13.74 8.42105 13.2 8.96241 13.2 9.62406V12.0301H16.8V15.6391H13.2V24C19.26 23.3985 24 18.2737 24 12.0301Z"
      fill="white"
    />
  </svg>
);

const InstagramIcon: FC<ArrowIconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="none"
    className={cn(className)}>
    <g clipPath="url(#clip0_26_182)">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M12 0C18.623 0 24 5.37703 24 12C24 18.623 18.623 24 12 24C5.37703 24 0 18.623 0 12C0 5.37703 5.37703 0 12 0ZM12 4.5C9.96314 4.5 9.70772 4.50863 8.90775 4.54514C8.10947 4.58156 7.56427 4.70836 7.08717 4.89375C6.594 5.08542 6.17573 5.34188 5.75878 5.75883C5.34183 6.17578 5.08538 6.59405 4.8937 7.08722C4.70831 7.56427 4.58156 8.10947 4.54509 8.90775C4.50863 9.70772 4.5 9.96314 4.5 12C4.5 14.0369 4.50863 14.2923 4.54509 15.0922C4.58156 15.8905 4.70831 16.4357 4.8937 16.9128C5.08538 17.406 5.34183 17.8242 5.75878 18.2412C6.17573 18.6582 6.594 18.9146 7.08717 19.1063C7.56427 19.2916 8.10947 19.4184 8.90775 19.4549C9.70772 19.4914 9.96314 19.5 12 19.5C14.0369 19.5 14.2923 19.4914 15.0922 19.4549C15.8905 19.4184 16.4357 19.2916 16.9128 19.1063C17.406 18.9146 17.8242 18.6582 18.2412 18.2412C18.6582 17.8242 18.9146 17.406 19.1063 16.9128C19.2916 16.4357 19.4184 15.8905 19.4549 15.0922C19.4914 14.2923 19.5 14.0369 19.5 12C19.5 9.96314 19.4914 9.70772 19.4549 8.90775C19.4184 8.10947 19.2916 7.56427 19.1063 7.08722C18.9146 6.59405 18.6582 6.17578 18.2412 5.75883C17.8242 5.34188 17.406 5.08542 16.9128 4.89375C16.4357 4.70836 15.8905 4.58156 15.0922 4.54514C14.2923 4.50863 14.0369 4.5 12 4.5ZM12 5.85136C14.0026 5.85136 14.2398 5.859 15.0307 5.89509C15.7619 5.92847 16.159 6.05062 16.4233 6.15333C16.7734 6.28936 17.0232 6.45192 17.2856 6.71438C17.5481 6.97678 17.7106 7.22667 17.8467 7.57673C17.9494 7.84102 18.0716 8.23809 18.1049 8.96934C18.141 9.76022 18.1486 9.99741 18.1486 12C18.1486 14.0026 18.141 14.2398 18.1049 15.0307C18.0716 15.7619 17.9494 16.159 17.8467 16.4233C17.7106 16.7734 17.5481 17.0232 17.2856 17.2856C17.0232 17.5481 16.7734 17.7106 16.4233 17.8467C16.159 17.9494 15.7619 18.0716 15.0307 18.1049C14.2399 18.141 14.0027 18.1486 12 18.1486C9.99727 18.1486 9.76013 18.141 8.96934 18.1049C8.23809 18.0716 7.84102 17.9494 7.57673 17.8467C7.22662 17.7106 6.97678 17.5481 6.71433 17.2856C6.45187 17.0232 6.28936 16.7734 6.15333 16.4233C6.05062 16.159 5.92842 15.7619 5.89505 15.0307C5.85895 14.2398 5.85136 14.0026 5.85136 12C5.85136 9.99741 5.85895 9.76022 5.89505 8.96934C5.92842 8.23809 6.05062 7.84102 6.15333 7.57673C6.28936 7.22667 6.45187 6.97678 6.71433 6.71438C6.97678 6.45192 7.22662 6.28936 7.57673 6.15333C7.84102 6.05062 8.23809 5.92847 8.96934 5.89509C9.76022 5.859 9.99741 5.85136 12 5.85136ZM12 8.14866C9.87295 8.14866 8.14866 9.87295 8.14866 12C8.14866 14.127 9.87295 15.8513 12 15.8513C14.127 15.8513 15.8513 14.127 15.8513 12C15.8513 9.87295 14.127 8.14866 12 8.14866ZM12 14.5C10.6193 14.5 9.50002 13.3807 9.50002 12C9.50002 10.6193 10.6193 9.50002 12 9.50002C13.3807 9.50002 14.5 10.6193 14.5 12C14.5 13.3807 13.3807 14.5 12 14.5ZM16.9035 7.9965C16.9035 8.49356 16.5006 8.89645 16.0035 8.89645C15.5065 8.89645 15.1035 8.49356 15.1035 7.9965C15.1035 7.49944 15.5065 7.0965 16.0035 7.0965C16.5006 7.0965 16.9035 7.49944 16.9035 7.9965Z"
        fill="white"
      />
    </g>
    <defs>
      <clipPath id="clip0_26_182">
        <rect width="24" height="24" fill="white" />
      </clipPath>
    </defs>
  </svg>
);

const TikTokIcon: FC<ArrowIconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="none"
    className={cn(className)}>
    <path
      d="M12 24C18.6274 24 24 18.6274 24 12C24 5.37258 18.6274 0 12 0C5.37258 0 0 5.37258 0 12C0 18.6274 5.37258 24 12 24Z"
      fill="white"
    />
    <mask
      id="mask0_26_190"
      style={{ maskType: 'luminance' }}
      maskUnits="userSpaceOnUse"
      x="4"
      y="4"
      width="17"
      height="17">
      <path d="M20.4 4.79999H4.8V20.4H20.4V4.79999Z" fill="white" />
    </mask>
    <g mask="url(#mask0_26_190)">
      <path
        d="M15.8987 10.4318C16.906 11.1515 18.1399 11.575 19.4726 11.575V9.01177C19.2204 9.01182 18.9689 8.9855 18.722 8.93324V10.9508C17.3894 10.9508 16.1556 10.5275 15.1482 9.80785V15.0387C15.1482 17.6555 13.0258 19.7765 10.4078 19.7765C9.43107 19.7765 8.52314 19.4814 7.76895 18.9751C8.62976 19.8548 9.83017 20.4006 11.1582 20.4006C13.7764 20.4006 15.8988 18.2795 15.8988 15.6627L15.8987 10.4318ZM16.8246 7.84589C16.3098 7.28379 15.9718 6.55739 15.8987 5.75429V5.42462H15.1874C15.3665 6.44532 15.9772 7.31733 16.8246 7.84589ZM9.42477 16.9672C9.13715 16.5904 8.9817 16.1292 8.98239 15.6551C8.98239 14.4583 9.95317 13.488 11.1509 13.488C11.374 13.4879 11.5958 13.5221 11.8086 13.5895V10.969C11.56 10.935 11.3092 10.9205 11.0584 10.9258V12.9654C10.8455 12.898 10.6236 12.8638 10.4003 12.864C9.20264 12.864 8.23192 13.8342 8.23192 15.0312C8.23192 15.8776 8.71712 16.6103 9.42477 16.9672Z"
        fill="white"
      />
      <path
        d="M15.1463 9.80785C16.1538 10.5274 17.3875 10.9508 18.7201 10.9508V8.93319C17.9762 8.77481 17.3178 8.38631 16.8227 7.84589C15.9751 7.31729 15.3646 6.44527 15.1855 5.42462H13.3172V15.6625C13.313 16.856 12.3439 17.8224 11.1488 17.8224C10.4446 17.8224 9.819 17.487 9.42274 16.9672C8.71519 16.6103 8.22995 15.8774 8.22995 15.0312C8.22995 13.8344 9.20065 12.864 10.3982 12.864C10.6278 12.864 10.849 12.8998 11.0563 12.9655V10.9259C8.48443 10.9789 6.41602 13.0794 6.41602 15.6625C6.41602 16.952 6.93108 18.1211 7.76708 18.9753C8.52127 19.4814 9.42914 19.7766 10.406 19.7766C13.024 19.7766 15.1463 17.6554 15.1463 15.0387V9.80785Z"
        fill="#8E000B"
      />
      <path
        d="M18.7213 8.93251V8.38708C18.0505 8.3881 17.3929 8.20033 16.8238 7.84526C17.3275 8.39652 17.991 8.77657 18.7213 8.93251ZM15.1866 5.42394C15.1694 5.32642 15.1564 5.22824 15.1472 5.12965V4.79999H12.5677V15.038C12.5636 16.2314 11.5945 17.1978 10.3993 17.1978C10.0606 17.1984 9.72635 17.1192 9.42377 16.9667C9.81995 17.4864 10.4456 17.8218 11.1499 17.8218C12.3449 17.8218 13.3141 16.8555 13.3183 15.6619V5.42399L15.1866 5.42394ZM11.0576 10.9251V10.3443C10.842 10.3149 10.6247 10.3002 10.4071 10.3003C7.78894 10.3003 5.6666 12.4215 5.6666 15.038C5.6666 16.6784 6.5007 18.1242 7.76823 18.9745C6.93224 18.1205 6.41717 16.9514 6.41717 15.6619C6.41717 13.0788 8.48553 10.9783 11.0576 10.9251Z"
        fill="#6C0008"
      />
    </g>
  </svg>
);

const YoutubeIcon: FC<ArrowIconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="none"
    className={cn(className)}>
    <path
      d="M12 0C5.376 0 0 5.376 0 12C0 18.624 5.376 24 12 24C18.624 24 24 18.624 24 12C24 5.376 18.624 0 12 0ZM12 19.104C8.076 19.104 4.896 15.924 4.896 12C4.896 8.076 8.076 4.896 12 4.896C15.924 4.896 19.104 8.076 19.104 12C19.104 15.924 15.924 19.104 12 19.104ZM12 5.772C8.568 5.772 5.772 8.568 5.772 12C5.772 15.432 8.568 18.228 12 18.228C15.432 18.228 18.228 15.432 18.228 12C18.228 8.568 15.432 5.772 12 5.772ZM9.684 15.54V8.46L15.816 12L9.684 15.54Z"
      fill="white"
    />
  </svg>
);

const SearchIcon: FC<ArrowIconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="none"
    className={cn(className)}>
    <g clipPath="url(#clip0_26_296)">
      <path
        d="M10.2857 0C15.9634 0 20.5714 4.608 20.5714 10.2857C20.5714 15.9634 15.9634 20.5714 10.2857 20.5714C4.608 20.5714 0 15.9634 0 10.2857C0 4.608 4.608 0 10.2857 0ZM10.2857 18.2857C14.7051 18.2857 18.2857 14.7051 18.2857 10.2857C18.2857 5.86629 14.7051 2.28571 10.2857 2.28571C5.86629 2.28571 2.28571 5.86629 2.28571 10.2857C2.28571 14.7051 5.86629 18.2857 10.2857 18.2857ZM19.9829 18.3669L25.5 23.8829L23.8829 25.5L18.3669 19.9829L19.9829 18.3669Z"
        fill="#8E000B"
        fillOpacity="0.8"
      />
    </g>
    <defs>
      <clipPath id="clip0_26_296">
        <rect width="24" height="24" fill="white" />
      </clipPath>
    </defs>
  </svg>
);

const HomeIcon: FC<ArrowIconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="none"
    className={cn(className)}>
    <path
      d="M9.02 2.84001L3.63 7.04001C2.73 7.74001 2 9.23001 2 10.36V17.77C2 20.09 3.89 21.99 6.21 21.99H17.79C20.11 21.99 22 20.09 22 17.78V10.5C22 9.29001 21.19 7.74001 20.2 7.05001L14.02 2.72001C12.62 1.74001 10.37 1.79001 9.02 2.84001Z"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path d="M12 17.99V14.99" stroke="#8E000B" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);

const ShoppingBagIcon: FC<ArrowIconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="none"
    className={cn(className)}>
    <path
      d="M3.01001 11.22V15.71C3.01001 20.2 4.81001 22 9.30001 22H14.69C19.18 22 20.98 20.2 20.98 15.71V11.22"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M12 12C13.83 12 15.18 10.51 15 8.68L14.34 2H9.66999L8.99999 8.68C8.81999 10.51 10.17 12 12 12Z"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M18.31 12C20.33 12 21.81 10.36 21.61 8.35L21.33 5.6C20.97 3 19.97 2 17.35 2H14.3L15 9.01C15.17 10.66 16.66 12 18.31 12Z"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M5.63988 12C7.28988 12 8.77988 10.66 8.93988 9.01L9.15988 6.8L9.63988 2H6.58988C3.96988 2 2.96988 3 2.60988 5.6L2.33988 8.35C2.13988 10.36 3.61988 12 5.63988 12Z"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M12 17C10.33 17 9.5 17.83 9.5 19.5V22H14.5V19.5C14.5 17.83 13.67 17 12 17Z"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
  </svg>
);
const StarIcon: FC<ArrowIconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="none"
    className={cn(className)}>
    <path
      d="M9.62012 16L11.1201 17.5L14.3701 14.5"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M8.80994 2L5.18994 5.63"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeMiterlimit="10"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M15.1899 2L18.8099 5.63"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeMiterlimit="10"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M2 7.85001C2 6.00001 2.99 5.85001 4.22 5.85001H19.78C21.01 5.85001 22 6.00001 22 7.85001C22 10 21.01 9.85001 19.78 9.85001H4.22C2.99 9.85001 2 10 2 7.85001Z"
      stroke="#8E000B"
      strokeWidth="1.5"
    />
    <path
      d="M3.5 10L4.91 18.64C5.23 20.58 6 22 8.86 22H14.89C18 22 18.46 20.64 18.82 18.76L20.5 10"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
    />
  </svg>
);

const TagIcon: FC<ArrowIconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="none"
    className={cn(className)}>
    <path
      d="M4.26001 11.02V15.99C4.26001 17.81 4.26001 17.81 5.98001 18.97L10.71 21.7C11.42 22.11 12.58 22.11 13.29 21.7L18.02 18.97C19.74 17.81 19.74 17.81 19.74 15.99V11.02C19.74 9.2 19.74 9.2 18.02 8.04L13.29 5.31C12.58 4.9 11.42 4.9 10.71 5.31L5.98001 8.04C4.26001 9.2 4.26001 9.2 4.26001 11.02Z"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M17.5 7.63V5C17.5 3 16.5 2 14.5 2H9.5C7.5 2 6.5 3 6.5 5V7.56"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M12.63 10.99L13.2 11.88C13.29 12.02 13.49 12.16 13.64 12.2L14.66 12.46C15.29 12.62 15.46 13.16 15.05 13.66L14.38 14.47C14.28 14.6 14.2 14.83 14.21 14.99L14.27 16.04C14.31 16.69 13.85 17.02 13.25 16.78L12.27 16.39C12.12 16.33 11.87 16.33 11.72 16.39L10.74 16.78C10.14 17.02 9.68002 16.68 9.72002 16.04L9.78002 14.99C9.79002 14.83 9.71002 14.59 9.61002 14.47L8.94002 13.66C8.53002 13.16 8.70002 12.62 9.33002 12.46L10.35 12.2C10.51 12.16 10.71 12.01 10.79 11.88L11.36 10.99C11.72 10.45 12.28 10.45 12.63 10.99Z"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
  </svg>
);

const HexagonIcon: FC<ArrowIconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="20"
    height="23"
    viewBox="0 0 20 23"
    fill="none"
    className={cn(className)}>
    <path
      d="M1 8.4312V14.5688C1 16.8164 1 16.8164 3 18.2489L8.5 21.6203C9.32558 22.1266 10.6744 22.1266 11.5 21.6203L17 18.2489C19 16.8164 19 16.8164 19 14.5688V8.4312C19 6.18362 19 6.18362 17 4.7511L11.5 1.37974C10.6744 0.87342 9.32558 0.87342 8.5 1.37974L3 4.7511C1 6.18362 1 6.18362 1 8.4312Z"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
  </svg>
);

const TruckIcon: FC<ArrowIconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="none"
    className={cn(className)}>
    <path
      d="M15 2V12C15 13.1 14.1 14 13 14H2V6C2 3.79 3.79 2 6 2H15Z"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M22 14V17C22 18.66 20.66 20 19 20H18C18 18.9 17.1 18 16 18C14.9 18 14 18.9 14 20H10C10 18.9 9.1 18 8 18C6.9 18 6 18.9 6 20H5C3.34 20 2 18.66 2 17V14H13C14.1 14 15 13.1 15 12V5H16.84C17.56 5 18.22 5.39001 18.58 6.01001L20.29 9H19C18.45 9 18 9.45 18 10V13C18 13.55 18.45 14 19 14H22Z"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M8 22C9.10457 22 10 21.1046 10 20C10 18.8954 9.10457 18 8 18C6.89543 18 6 18.8954 6 20C6 21.1046 6.89543 22 8 22Z"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M16 22C17.1046 22 18 21.1046 18 20C18 18.8954 17.1046 18 16 18C14.8954 18 14 18.8954 14 20C14 21.1046 14.8954 22 16 22Z"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M22 12V14H19C18.45 14 18 13.55 18 13V10C18 9.45 18.45 9 19 9H20.29L22 12Z"
      stroke="#8E000B"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
  </svg>
);

const HamburgerIcon: FC<ArrowIconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="18"
    viewBox="0 0 24 18"
    fill="none"
    className={cn(className)}>
    <line x1="1" y1="1" x2="23" y2="1" stroke="#8E000B" strokeWidth="2" strokeLinecap="round" />
    <line x1="1" y1="9" x2="23" y2="9" stroke="#8E000B" strokeWidth="2" strokeLinecap="round" />
    <line x1="1" y1="17" x2="23" y2="17" stroke="#8E000B" strokeWidth="2" strokeLinecap="round" />
  </svg>
);

export {
  FacebookIcon,
  HamburgerIcon,
  HexagonIcon,
  HomeIcon,
  InstagramIcon,
  SearchIcon,
  ShoppingBagIcon,
  StarIcon,
  TagIcon,
  TikTokIcon,
  TruckIcon,
  YoutubeIcon,
};
